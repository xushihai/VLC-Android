...
class VideoGridFragment : MediaBrowserFragment<VideosViewModel>(), SwipeRefreshLayout.OnRefreshListener, Observer<PagedList<AbstractMediaWrapper>>, CtxActionReceiver {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!::videoListAdapter.isInitialized) {
            ...
            val grouping = arguments?.getSerializable(KEY_GROUPING) as VideoGroupingType?
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        val displayInCards = Settings.getInstance(requireActivity()).getBoolean("video_display_in_cards", false)
    ...
    override fun getTitle() = when (viewModel.groupingType) {
        VideoGroupingType.NONE -> viewModel.folder?.title ?: viewModel.group?.title
...
//        binding.videoGrid.setNumColumns(1)
//        binding.videoGrid.setBackgroundColor(Color.parseColor("#303030"))
//        binding.videoGrid.setNumColumns(if (listMode) 1 else -1)
