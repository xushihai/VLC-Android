...
class VideoListAdapter internal constructor(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        //todo 明天需要将文件夹形式的列表item与文件的列表item区分开来，文件夹的列表item还是用之前的布局
        var layoutId = R.layout.video_grid_card
        if (isListMode) {
            if (viewType == 1)
                layoutId = R.layout.videos_list_card
            else
                layoutId = R.layout.video_list_card
        }
...
//    }
    ...
    constructor(binding: ViewDataBinding) : SelectorViewHolder<ViewDataBinding>(binding), View.OnFocusChangeListener {
        ...
        val thumb: ImageView = itemView.findViewById(R.id.ml_item_thumbnail)

        init {
            binding.setVariable(BR.holder, this)
//            binding.setVariable(BR.cover, UiTools.getDefaultVideoDrawable(itemView.context))
            thumb.setBackgroundResource(R.drawable.ic_browser_video_default)
