...
object ThumbnailsProvider {
    ...
    fun getVideoThumbnail(media: AbstractMediaWrapper, width: Int): Bitmap? {
        ...
        val bitmap = synchronized(lock) {
            createVideoThumbnail(filePath, width)
//            ThumbnailUtils.createVideoThumbnail(filePath, MediaStore.Video.Thumbnails.MINI_KIND)
    ...
    fun createVideoThumbnail(filePath: String, vWidth: Int): Bitmap? {
...
//            bitmap = retriever.getFrameAtTime(-1)
            ...
            while (true) {
                ...
                if (us == -1L)
                    us = 0
                if (isFullBlack(bitmap)) {
                    us += (5 * 1000 * 1000).toLong()
                    if (count >= 10) {
                        break
                    }
                } else {
        ...
        if (width > vWidth) {
            val scale = vWidth * 1.0 / width
            val w = Math.round(scale * width).toInt()
            val h = Math.round(scale * height).toInt()
    ...
    fun isFullBlack(bitmap: Bitmap?): Boolean {
        if (bitmap == null)
            return false
        ...
        if (width == 0 || height == 0)
            return false
        val maxGap = Math.min(width, height) / 2
        val stideLength = Math.min(maxGap, 5)
        for (i in 0 until stideLength) {
            for (j in 0 until stideLength) {
        ...
        for (i in width - 1 downTo width - 1 - stideLength + 1) {
            for (j in 0 until stideLength) {
                val color = bitmap.getPixel(i, j)
                if (color != Color.BLACK) {
                    isFullBlack = false
        ...
        for (i in 0 until stideLength) {
            for (j in height - 1 downTo height - 1 - stideLength + 1) {
                val color = bitmap.getPixel(i, j)
                if (color != Color.BLACK) {
                    isFullBlack = false
                    break
    ...
    suspend fun getPlaylistImage(key: String, mediaList: List<AbstractMediaWrapper>, width: Int) =
            (BitmapCache.getBitmapFromMemCache(key)
                    ?: composePlaylistImage(mediaList, width))?.also {
